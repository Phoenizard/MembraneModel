...
库内核(`model` / `solver` / `discretization`)与驱动(`run_IEQ` / `visualize`)并列在根目录。

## 跑一次实验

```bash
# smoke:32^3 小算例,前台,实时进度
python -u run_IEQ.py --quick --name smoke --log 20

# 正式算例:后台 + 写日志文件
python -u run_IEQ.py --N 32 --eps 0.1964 --k 1.0 --delta 10 \
...
`v_d/a_0/a_d`(几何目标)与 continuation 的 `M_start/M_max/rho` 目前写死在 `run_IEQ.py`,需改这些再编辑脚本。
